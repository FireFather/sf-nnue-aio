...
namespace {
  ...
  Bitboard sliding_attack(const PieceType pt, const Square sq, Bitboard occupied) {
    ...
    for(const Direction d : pt == ROOK ? RookDirections : BishopDirections)
    {
        Square s = sq;
  ...
  void init_magics(const PieceType pt, Bitboard table[], Magic magics[]) {
    ...
    for (Square s = SQ_A1; s <= SQ_H8; ++s)
    {
        ...
        for (int i = 0; i < size; )
        {
            ...
            {
	            if (unsigned idx = m.index(occupancy[i]); epoch[idx] < cnt)
                {
                    epoch[idx] = cnt;
