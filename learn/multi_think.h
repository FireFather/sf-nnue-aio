...
struct MultiThink
{
	...
	// ・Book is not thread safe in on the fly mode, so this mode is temporarily
	// Turn it off.
	// [Requirements]
	...
	// It is good to read the fixed trace at that timing.
	virtual void init() {}

	// A thread worker that is called by creating a thread when go_think()
	...
	// [ASYNC] Retrieve the value of the loop counter and add the loop counter after retrieval.
	// If the loop counter has reached loop_max, return UINT64_MAX.
	// If you want to generate a phase, you must call this function at the timing of generating the phase,
...
private:
	...
	// Thread end flag.
	// vector<bool> may not be reflected correctly when trying to rewrite from multiple threads...
	typedef uint8_t Flag;
	std::vector<Flag> thread_finished;

};

// Mechanism to process tasks during idle time.
...
// Convenient to use when you want to write thread worker of MultiThink in master-slave method.
