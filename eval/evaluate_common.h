...
// A common header like modern evaluation function (EVAL_KPPT and EVAL_KPP_KKPT).
...
namespace Eval
{
	...
	// type represents the survey target.
	// type = -1 :all KK,KKP,KPP
	// type = 0: KK only
	// type = 1: KKP only
	// type = 2: KPP only
...
// After eta2_epoch, it gradually changes from eta2 to eta3.
	...
	// Function to normalize KK. Note that it is not completely equivalent to the original evaluation function.
	// By making the values ​​of kkp,kpp as close to zero as possible, the value of the feature factor (which is zero) that did not appear during learning
